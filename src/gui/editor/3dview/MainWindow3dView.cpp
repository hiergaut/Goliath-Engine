        ...
        case Qt::Key_F:
            if (!m_shiftPressed) {
                ...
                case CameraStrategy::FPS:
                    ...
                    m_camera->m_cameraStrategy = new CameraWorld(position, target, m_camera->getTransform(), m_camera->m_id, *m_camera->getModel());
                ...
                case CameraStrategy::WORLD:
                    ...
                    //                    delete m_camera;
                    delete m_camera->m_cameraStrategy;
                    m_camera->m_cameraStrategy = new CameraFps(position, yaw, pitch, this, m_camera->getTransform(), m_camera->m_id, m_camera->m_fov, *m_camera->getModel());
...
//        m_fun->glGenerateMipmap(GL_TEXTURE_2D);
    ...
    unsigned int attachments[2] = { GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1 };
    m_fun->glDrawBuffers(2, attachments);
    ...
    for (unsigned int i = 0; i < 2; i++) {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // we clamp to the edge as the blur filter would otherwise sample repeated texture values!
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        m_fun->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_pingpongColorBuffers[i], 0);
...
void MainWindow3dView::updateGL()
{
    ...
    //    unsigned int colorBuffers[2];
    //    glGenTextures(1, &m_colorBuffers);
    for (unsigned int i = 0; i < 2; i++) {
        glBindTexture(GL_TEXTURE_2D, m_colorBuffers[i]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, width(), height(), 0, GL_RGB, GL_FLOAT, NULL);
    }
    ...
    //    glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);
    // attach texture to framebuffer
    //    m_fun->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_colorBuffers, 0);
    //    }
    //    glDrawBuffer(GL_NONE);
    //    glReadBuffer(GL_NONE);
...
void MainWindow3dView::updateAllFrutumViews()
{
